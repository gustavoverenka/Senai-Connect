...
const reportSchema = z.object({ reason: z.enum(['spam', 'ofensivo', 'conteudo_improprio', 'desinformacaçao', 'outro'], {
...
const reportPost = async (req, res) => {
    ...
    try {
        ...
        if (!existingReport.empty) {
            return res.status(400).json({ error: 'Voce ja enviou uma denuncia para esta publicacao.'});
        }
        ...
        return res.status(201).json({ message: 'Denuncia enviada com sucesso para equipe de moderação.',
            reportId: docRef.id
        });
    } catch (error) {
